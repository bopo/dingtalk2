...
if __name__ == '__main__':
    # import doctest
    #
    # xiaoding = DingTalk(access='token', secret='secret', pc_slide=True, fail_notice=False)
    # doctest.testmod()

    talk = DingTalk(access='77eb420ff2761ad516d974e1428c3e198b84faabc9c9ef8e86b2c71ac60bd0ea', secret=None, pc_slide=False, fail_notice=False)
    talk.send(action='text', **dict(msg='[监控]哈哈'))
    # talk.send(action='link', **{})
    # talk.send(action='image', **{})
    # talk.send(action='markdown', **{})
    # talk.send(action='action_card', **{})
    # talk.send(action='feed_card', **{})
